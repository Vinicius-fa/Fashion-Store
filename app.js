...
async function loadProductDetails() {
    ...
    if (!idDoProduto) {
        container.innerHTML = "<h2>Produto não especificado.</h2>";
        return;
    }

    try {
        const response = await fetch(`${BASE_URL}/products/${idDoProduto}`);
        if (!response.ok) throw new Error("Produto não encontrado");
        
        const product = await response.json();

        container.innerHTML = `
            <img src="${product.images[0]}" alt="${product.title}" class="detail-img" onerror="this.src='https://placehold.co/600x400?text=Imagem+Indisponível'">
        ...
        container.innerHTML = "<h2>Erro ao carregar os detalhes do produto.</h2>";
    }
}